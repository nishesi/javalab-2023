...
public class HotelServiceImpl implements HotelService {
    ...
    public HotelServiceImpl(OkHttpClient httpClient,
    ...
    @Override
    public Page<HotelDto> getCinemasInCity(String city, Pageable pageable) {
        String text = "отели, " + city;

        Request.Builder request = new Request.Builder();
        request.setUrl$okhttp(url.newBuilder().addQueryParameter("text", text).build());
        Call call = httpClient.newCall(request.build());

        YandexMapsResponse<HotelDto> resp = parseResponse(call, new TypeReference<>() {
        });
